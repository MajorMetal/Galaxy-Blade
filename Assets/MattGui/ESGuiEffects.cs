...
public class ESGuiEffects : MonoBehaviour {
	...
	void OnTriggerEnter(Collider col) {
		...
		if (col.gameObject.tag == "Player" && anim.GetBool("isBlocking") == false)
		{
			StartCoroutine("DoTheDance"); 
		}
		if (col.gameObject.tag == "PlayerShield" && anim.GetBool("isBlocking"))
		{
 ...
 public void ColorChange()
	{
	  if (block)
	  {
		blockedText.color = Color.Lerp (blockedText.color, Color.blue, fadeSpeed * Time.deltaTime);
	  }	
	  ...
	  if (hit)
	  {
		hitText.color = Color.Lerp (hitText.color, Color.red, fadeSpeed * Time.deltaTime);
	}
	  if (!hit)
	  {
